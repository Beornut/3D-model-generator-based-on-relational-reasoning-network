...
import utils
import math
from clevr_dataset_connector import ApplyDataset, ClevrDataset, ClevrDatasetStateDescription
...
def apply(args):
    ...
    qsts, objs = utils.build_questions()
    args.qdict_size = len(dictionaries[0])
    args.adict_size = len(dictionaries[1])
    yes_idx = dictionaries[1]['yes'] - 1
    ...
    test_transforms = transforms.Compose([transforms.Resize((128, 128)),
                                          transforms.ToTensor()])
    apply_dataset = ApplyDataset(args.path, qsts, dictionaries, test_transforms)
    apply_dataset_loader = DataLoader(apply_dataset, batch_size=len(qsts),
                                      shuffle=False, num_workers=8, collate_fn=utils.collate_apply_dataset)
    for batch_idx, sample_batched in enumerate(apply_dataset_loader):
        args.res = []
        img, qst = utils.load_apply_data(sample_batched, args.invert_questions)
        output = model(img, qst)
        pred = output.data.max(1)[1]
        for k, v in enumerate(pred):
            if v == yes_idx:
                args.res.append(objs[k][0])
                args.res.append(objs[k][1])
                args.res.append(objs[k][2])
        args.res = utils.load_dictionaries(args)
        print(utils.output_model(args))
        print(args.res)
        return args.res
...
def test(data, model, epoch, dictionaries, args):
    ...
    sorted_classes = sorted(dictionaries[2].items(),
                            key=lambda x: hash(x[1]) if x[1] != 'number' else int(inverted_answ_dict[x[0]]))
    ...
    print('Test Epoch {}: Accuracy = {:.2%} ({:g}/{}); Invalids = {:.2%} ({:g}/{}); Test loss = {}'.format(epoch,
                                                                                                           accuracy,
                                                                                                           corrects,
                                                                                                           n_samples,
                                                                                                           invalids_perc,
    ...
    for v in class_n_samples.keys():
        ...
        print('{} -- acc: {:.2%} ({}/{}); invalid: {:.2%} ({}/{})'.format(v, accuracy, class_corrects[v],
                                                                          class_n_samples[v], invalid,
...
def main(args):
    ...
    if args.test:
        ...
        _, clevr_test_loader = reload_loaders(clevr_dataset_train, clevr_dataset_test, args.batch_size,
        ...
        for epoch in progress_bar:
            if (((args.bs_max > 0 and bs < args.bs_max) or args.bs_max < 0) and (
                    epoch % args.bs_step == 0 or epoch == start_epoch)):
                bs = math.floor(args.batch_size * (args.bs_gamma ** (epoch // args.bs_step)))
                if bs > args.bs_max and args.bs_max > 0:
                    bs = args.bs_max
                clevr_train_loader, clevr_test_loader = reload_loaders(clevr_dataset_train, clevr_dataset_test, bs,
                                                                       args.test_batch_size, hyp['state_description'])
...
if __name__ == '__main__':
    ...
    parser.add_argument('--visualize', action='store_true', default=False,
                        help='visualize the model of result vector in a window created by VTK --visualize')
    parser.add_argument('--obj', action='store_true', default=False,
                        help='output the 3D model file with format of obj --obj')
    parser.add_argument('--path', type=str, default='input.png',
                        help='the path of the image file to be recognized')
